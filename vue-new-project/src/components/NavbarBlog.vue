<template>
  <header class="nav_header bg-gray-800" :data-admin="admin">
    <div class="w-full flex justify-between">
      <div
        class="flex justify-center items-center space-x-0 xsm:space-x-1 text-sm md:text-lg"
      >
        <img
          alt="generic logo"
          class="generic_logo"
        ...
        <RouterLink class="btn btn-logo" to="/">
          <div>
            Bl<span class="text-blue-800">o</span>gPan<span
      ...
      <div class="mini_admin_logo" v-if="admin">
        <span class="text-white bg-green-600">admin</span>
      </div>

      <nav
        v-if="
          currentUserIn !== null &&
          currentUserIn !== undefined &&
          admin === false
        "
      ...
      >
        ...
        >
          ...
          <div class="init_post w-24">
            <RouterLink
              id="init_new_post"
              class="btn_new_post w-full h-full text-white rounded-2xl"
              to="/create"
            >
              create a post
            </RouterLink>
        ...
        <button class="btn-logout-desk" @click.prevent="logoutSession">
          Logout
        </button>
      </nav>
          ...
          <li class="w-full h-full">
            <div class="filter_wrapper z-10">
              <div class="filter_box">
                <span>Filter</span>
                <div
                  class="text-white flex items-center cursor-pointer z-20"
                  @click="switchFilter"
                >
                  <div class="thin_bar"></div>
                  <div class="arrow_filter_wrap mx-2">
                    <span
                      class="arrow_filter relative"
              ...
              <div class="section_search relative md:top-1">
                <ul class="search_content">
                  <li class="w-9/12 h-full">
                    <input
                      type="text"
                      class="text_search w-full h-full flex justify-center items-center text-sm bg-gray-800/60 text-gray-400 outline-none"
                  ...
                  <li
                    class="w-3/12 bg-purple-900 h-full flex p-1 cursor-pointer z-10"
                    @click="submitSearch"
                  >
      ...
      >
        <button
          class="btn btn-link bg-purple-900"
          to="/login"
        ...
        <button
          class="btn btn-link bg-gray-700"
          to="/register"
        ...
        <div
          id="custom_feature"
          class="custom_features pl-1 flex flex-col justify-center items-center gap-2"
        >
          <ul
            class="panel_desk_custom flex text-gray-300 justify-center items-center gap-1"
          >
            <li class="relative">Custom</li>
            <li
              class="relative md:top-1 font-bold cursor-pointer"
              @click="handleCustom"
            >
              &#65088;
            </li>
            ...
            <div class="desk_feature_wrapper flex-col text-white gap-2">
              ...
              <div class="w-full flex" v-if="minicustom">
                <div class="select_blog z-10">
                  <label for="all" style="font-size: calc(14px + 0.32vw)"
                    >all</label
                  >
                ...
                <div class="select_blog z-10">
                  <label for="single" style="font-size: calc(14px + 0.32vw)"
                    >single</label
                  >
                  <div class="box_circle w-10 grid place-items-start">
                    <input
      ...
      >
        <div class="grid place-items-center">
          <p class="user_in_mob text-xl xsm:text-base text-white underline">
            {{ shortyName }}
          </p>
        </div>
        <ul
          id="menu_wrap"
          class="menu_wrap flex xxsm:flex-col xsm:flex-row justify-center gap-2 bg-gray-700 rounded"
          style="padding: 5px"
        >
          <li>
            <img
        ...
        <div class="others_features flex flex-col rounded-md" v-if="custom">
          ...
          <div class="p-2 text-lg xsm:text-base">
            ...
            <div
              class="written_wrapper w-full flex justify-between items-center h-8"
            >
              <span class="articles_label text-gray-500 font-bold"
                >Articles</span
              >
              <span
                class="count_articles w-3 text-center rounded-2xl bg-green-800"
                >{{ countArt }}</span
              >
            </div>
            <button class="btn-new-account" @click="redirectLoginPage">
              log another account
            </button>
          ...
          <div class="custom_mob_wrap w-full text-white">
            <p class="custom-p xsm:text-base">
              <span>Custom</span>
              <span
                class="relative top-2 cursor-pointer"
                @click="handleMiniCustom"
                >&#65088;</span
              >
            </p>
      ...
      >
        <div class="grid place-items-center">
          <p class="user_in_mob text-sm xsm:text-base text-white underline">
            {{ shortyName }}
          </p>
        </div>
        <ul
          id="menu_wrap"
          class="menu_wrap flex xxsm:flex-col xsm:flex-row justify-center items-center gap-2 bg-gray-700 rounded"
          style="padding: 5px"
        >
        ...
        <div class="others_features flex flex-col rounded-md" v-if="custom">
          <div class="w-full flex justify-between px-1">
            <div
              class="aside_login_label w-3/12 flex justify-start items-center"
            >
              <p
                class="aside_login_label w-12 text-sm grid place-items-center text-gray-300 bg-purple-900"
                style="
          ...
          <div class="main_items" style="padding: 0.25rem 0.25rem 0.5rem">
            <p class="user_in">welcome {{ shortyName }}</p>
            <p v-if="lastDateAction !== undefined" class="last_post">
              last Post:{{ lastDateAction }}
            </p>
            ...
            <div id="custom_feature" class="custom_features text-gray-300">
              <div class="filter_wrapper flex flex-column" v-if="admin">
                <div class="filter_box">
                  <span>Filter</span>
                  <div
                    class="text-white flex items-center cursor-pointer z-20"
                  ...
                  >
                    <div class="thin_bar"></div>
                    <div class="arrow_filter_wrap mx-2">
                      <span class="arrow_filter relative">&#x25BE;</span>
                    </div>
                  </div>
                </div>
                <div
                  class="filter_box_mobi_selection hide_selection"
                  ref="filterBox"
                >
                  <div
                    class="filter_box_mobi_content w-full flex flex-col gap-3 justify-center items-center font-bold text-gray-500 z-10"
                    style="font-size: calc(14px + 0.28vw)"
                    @click="handleFilterSelection"
                  >
                    <div
                      class="control_selection w-full flex justify-between items-center"
              ...
              <div class="section_search" v-if="admin">
                <ul
                  class="search_content_mob w-full h-7 flex justify-start items-center p-1"
                >
                  <li class="relative h-full bg-gray-800/90">
                    <input
                      type="text"
                      class="text_search w-10/12 h-full text-sm text-white outline-none z-10"
          ...
          <div class="custom_mob_wrap w-full text-white">
            <p class="custom-p">
              <span>custom</span>
              <span @click="handleMiniCustom">&#65088;</span>
            </p>
            <div class="gather_custom w-full text-sm gap-1" v-if="minicustom">
              ...
              <span class="block" style="margin: 1.5rem 0 0.25rem"
                >Features</span
              >
      ...
      >
        ...
        <div
          class="others_features flex flex-col rounded-md gap-3"
          v-if="custom"
        >
          <button
          ...
          <div class="custom_mob_wrap w-full text-white">
            <p class="custom-p">
              <span>Custom</span>
              <span
                class="relative top-2 left-1 cursor-pointer"
                @click="handleMiniCustom"
                >&#65088;</span
              >
            </p>
...
function handleRadioState(e) {
  ...
  if (e.target.id === "all_blogr" || e.target.id === "all_blogger") {
    ...
    e.target.id === "single_blogr" ||
    e.target.id === "single_blogger"
  ) {
...
async function redirectLink(e, label) {
  ...
  if (label === "login") {
    ...
    if (newUserInfo !== "null") {
      ...
      newUserInfo.admin
        ? userStore.$patch({ isAdmin: true })
        : userStore.$patch({ isAdmin: false });
      router.push({ path: "/" });
    } else {
...
async function submitSearch() {
  ...
  if (pickMsg.value === "look for ...") {
    newPosts = await fetchspecificarticlesapi(
      "theme",
    ...
    const thisAuthor = allAuthors.find(
      (author) => author.username === inputSearch.value.value
    );
    newPosts = await fetchspecificarticlesapi(
      "author",
      inputSearch.value.value,
...
@media (min-width: 180px) {
  ...
  p.user_in {
    @apply text-gray-300 transition-all duration-1000 ease-in-out hover:text-gray-500 py-2;
    position: relative;
  ...
  .btn-link {
    @apply text-white text-center w-28 py-1 my-auto rounded-md font-semibold hover:bg-green-500;
  }
  ...
  .btn-logout-desk {
    @apply cursor-pointer text-gray-300 underline;
  ...
  .custom-p {
    ...
    @apply flex items-center gap-1 font-bold h-4;
  }
  ...
  .thin_bar + .arrow_filter_wrap > span {
    top: 0.15rem;
    left: 2px;
  }
...
@media (min-width: 600px) {
  ...
  .nav_header[data-admin="true"] {
    height: 7.4rem;
    padding: 2.4rem 0.5rem 1rem !important;
    z-index: 2;
  }
  ...
  .nav_desktop_admin .custom_features {
    position: absolute;
    top: -1.25rem;
    right: 0.9rem;
    width: max-content;
